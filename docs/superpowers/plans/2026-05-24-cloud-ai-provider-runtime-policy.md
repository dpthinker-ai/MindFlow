...
Result: emulator validation completed after starting the existing MindFlow validation AVD without wipe/clear flags. Before install, a local data backup was saved under `recovery/`. `adb install -r app/build/outputs/apk/release/app-release.apk` succeeded, the app launched to `com.mindflow.app/.MainActivity`, existing notes remained present, local model state stayed unchanged, and the cloud AI settings page showed DeepSeek with `https://api.deepseek.com` and `deepseek-v4-flash`.

...
- `Direction Wiki`
  - 负责为“关注方向”积累长期 markdown 资产
  - 记录外部研究、证据分层、已验证结论、阶段快照和方向资产
  - 由后台 agent 工作流维护，而不是在移动端前台直接生成

这个 `Direction Wiki` 第一版已经实现为本地生成的 markdown 资产层。它不替代本地数据库，而是给 `Flow / 线程 / 提醒` 提供更稳定、更长期的方向资产。
...
- Flow：
  ...
  - `Flow` 里的方向也会露出 `Direction Wiki` 回流的证据基础，如 `外部视角 / 待验证 / 已查证 / 已验证`
  ...
  - `Direction Wiki` 回流会在这里明确区分 `证据基础 / 待验证 / 已查证 / 已验证`
  - `Direction Wiki` 还会回流基于历史快照累积出来的 `长期阶段 / 快照节奏`
...
这是已落地的第一版长期知识层，后续还会继续加强。

目标：
- 给关注方向建立一套长期积累的 markdown wiki
...
- Wiki 只服务“关注方向”，不是服务所有普通记录
- 原始输入追加到 `raw/`，不做静默覆盖
- Agent 维护 `wiki/` 页面、索引和日志
- App 通过轻量导出物读取 `方向资产 / 已验证结论 / 阶段历史摘要 / 开放问题`

当前第一版包含：
- 本地 `direction-wiki/` 目录生成
- `raw/`、`wiki/directions/`、`wiki/evidence/`、`wiki/snapshots/`、`wiki/index.md`、`wiki/log.md`
- `wiki/concepts/` 概念页，由重复出现的方向标签自动聚合生成
- 设置里的手动 `更新方向知识库`
- App 退到后台时的低频自动刷新
- `Flow` 和线程页对方向资产、证据基础、开放问题、阶段历史摘要的回流展示
- `Flow` 和线程页还会回流 Direction Wiki 的长期快照阶段与快照节奏

...
当前已经实现的是这个知识层的第一块：一个本地生成、以“关注方向”为中心的 direction-focused slice。它不替代本地数据库，而是先给 `Flow / 线程 / 提醒` 提供更稳定、更长期的方向资产。
...
- ingest 不只处理方向摘要，还要把原始记录、研究、验证、反思都作为 source
- 知识层不只维护 `direction` 页，还要维护 `concept / question / method / experiment` 等对象
- 查询和 AI 综合结果要继续 file back 回知识层，而不只是停在 UI 卡片里

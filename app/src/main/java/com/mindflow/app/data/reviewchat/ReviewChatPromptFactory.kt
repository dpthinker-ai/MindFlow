        ...
        when (packet.questionMode) {
            ...
            ReviewChatQuestionMode.COLLECTION_OVERVIEW -> {
                ...
                if (packet.wantsCategories) {
                    ...
                    appendLine("6. 类别只能来自命中原始记录的内容主题，不要把“时间范围”“统计信息”“历史记录”“查询结果”“集合概览”当成类别。")
                    appendLine("7. 不要输出 `【依据】` 或 `【下一步】`，也不要输出 Markdown 表格。")
                } else {
            ...
            ReviewChatQuestionMode.RECORD_LOOKUP -> {
                ...
                if (packet.wantsCategories) {
                    ...
                    appendLine("6. 优先综合“分类候选”的所有批次，再参考原始记录，不要只根据少量示例记录下结论。")
                    appendLine("7. 如果用户没有明确要求逐条列记录，就不要追加 `【记录】`。不要输出 `【依据】` 或 `【下一步】`，也不要输出 Markdown 表格。")
                } else {
            ...
            systemInstruction = buildString {
                ...
                when (packet.questionMode) {
                    ...
                    ReviewChatQuestionMode.RECORD_LOOKUP -> {
                        ...
                        if (packet.wantsCategories) {
                            appendLine("输出协议：先写 `【答复】`，再写 `【类别】`，每条都用 `- 类别：包含的信息`。类别只能来自命中原始记录的内容主题，不要把“时间范围”“统计信息”“历史记录”“查询结果”“集合概览”“确定结果”当成类别。优先综合“分类候选”的所有批次，再参考原始记录，不要只根据少量示例记录下结论。如果用户没有明确要求逐条列记录，就不要写 `【记录】`。不要输出 `【依据】` 或 `【下一步】`，也不要输出表格。")
                        } else {
